...
#define Nmax 1000			/*2000000*/ /* total number of iterations */
#define stepskip 100			/*2500*/ /* output graphics every stepskip steps */
...
void computebinder(void);
double wrap_nematic_angle_difference(double theta);
double computedefectdensity(void);
...
//#define alpha4phi 0.001 //0.07
//#define Kphi 0.01 // 0.01 // 0.14
double alpha4phi;
double Kphi;

double M = 1.0; //0.03;				/*0.05*/ /* need to tweak */
double radius = 40; //!!!! of what

double noisestrength=0.0;

double phi0; /*initial value of the concentration*/
...
/* switch for initial configuration*/
...
double n_director[Lx][Ly][Lz][3]; 
double theta[Lx][Ly]; /*calculation only for 2D*/
...
/*
 *	Leslie-Ericksen //for polarization
 */
...
double Abulk = 0.25; //1.0; /*determines bulk liquid crystal free energy A0 in the eq*/
double L1 = 0.01; //0.001 /*liquid crystal elastic constant*/
double W; // -0.01 /*anchoring term at interfaces*/
double phivr = 1.0; // hydrodynamics switch
double gammac;

//#define gamma 1.5 // 1.0 /*baseline value of gamma - N/I transition at 2.7*/
double gamma0;
#define delta 1.0 // 0.5

double energy;
double total_energy;
double anchoring_energy;
double U;
double avg_d;
...
int main(int argc, char** argv)
{
	...
	int ll,mm,nn;
        double uxl,uyl,uzl;
        double v0,v1,v2,dmax,mul,dens,phil,fpl,cl;
	...
	phi0 = atof(argv[1]);
	gamma0 = atof(argv[2]);
	Kphi = atof(argv[3]); //0.01 when nonzero
	W = atof(argv[4]); // 0.0;
	alpha4phi = atof(argv[5]);
	movie = atoi(argv[6]);
	seed=atoi(argv[7]);
	/*input pars*/
...
#if READFROMFILE
	fpread=fopen("startingprofile.dat","r");

	for (i=0;i<Lx;i++){
	  for (j=0;j<Ly;j++){
	    for (k=0;k<Lz;k++){
	      fscanf(fpread, "%d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",
		     &ll, &mm, &nn,
		     &Qxxl, &Qxyl, &Qxzl, &Qyyl, &Qyzl, &uxl, &uyl, &uzl,
		     &v0, &v1, &v2, &dmax, &mul, &dens, &phil, &fpl, &cl);
	    
			 double amp = 0.1;
		      Qxx[ll][mm][nn]=Qxxl+amp*(2.0*drand48()-1.0);
		      Qxy[ll][mm][nn]=Qxyl+amp*(2.0*drand48()-1.0);
	    	  Qxz[ll][mm][nn]=Qxzl+amp*(2.0*drand48()-1.0);
	    	  Qyy[ll][mm][nn]=Qyyl+amp*(2.0*drand48()-1.0);
	    	  Qyz[ll][mm][nn]=Qyzl+amp*(2.0*drand48()-1.0);

	    	  phi[ll][mm][nn]=phil+amp*(2.0*drand48()-1.0);
	    }
	  }
	}
...
fclose(fpread);
	...
	sprintf(filename2, "energy_phi0_%0.3f_gamma0_%0.3f_k_%0.3f_W_%0.3f.txt",phi0,gamma0,Kphi,W);
	sprintf(filename3, "binderD_phi0_%0.3f_gamma0_%0.3f_k_%0.3f_W_%0.3f.txt",phi0,gamma0,Kphi,W);
	...
	for (n = 1; n <= Nmax; n++) {
		...
		if (n % stepskip == 0) {
		  ...
		  // plot energy
		  computebinder();
		  fprintf(output2, "%d %E %E %E\n", n, energy, anchoring_energy, total_energy);
		  fprintf(output3, "%d %E %E\n", n, U, avg_d);
		}
		/**************************************************************/
	}
	...
	plotQ(n+1);
	printf("final Vy = %E \n \n", Vy);
	printf("Initial phi0 = %E | gamma0 = %E \n \n", phi0, gamma0);
	printf("final composition phi = %E \n \n", phi_average);
	fclose(output2);
	fclose(output3);
}
	...
	for (i = 0; i < Lx; i++) {
		for (j = 0; j < Ly; j++) {
			for (k = 0 ; k < Lz; k++) {
				...
				double ky = 1.0;
				// lines
				phi[i][j][k] = 1 + cos(kx*i+ky*j+2.0);
				//if (j>45 || j<2) phi[i][j][k] = 0;

				// bend
				// double B = 7;
				// double sigma = 25;

				// double bend = B*exp((-i*i-j*j)/(2*sigma*sigma));
				// phi[i][j][k] = 1+cos(bend + kx*i + ky*j + 10);
				// if (j<-i+36 || j<-2*i+46 || 2*j<-i+46 || j>-i+56) phi[i][j][k] = 0;
				// if (j<-i+25)  phi[i][j][k] = 1 + cos(kx*i + ky*j-10);
				// if (j>-i+69)  phi[i][j][k] = 1 + cos(kx*i + ky*j-0.5);

				//ring
				// double xc = 27;
				// double yc = 25;
				// //eliminate for vertical
				// if (i<15){				
				// 	phi[i][j][k] = 1+cos(kx*i + ky*j);	
				// }
				// if (i>40){				
				// 	phi[i][j][k] = 1+cos(kx*i + ky*j+5);	
				// }

				// // build the ring
				// if (((i-xc)*(i-xc) + (j-yc)*(j-yc))<160 && ((i-xc)*(i-xc) + (j-yc)*(j-yc))>60){
				// 	double r = sqrt((i-xc)*(i-xc)+(j-yc)*(j-yc));
				// 	phi[i][j][k] = 1+cos(1.8*kx*r-10);
				// }
				// if (((i-xc)*(i-xc) + (j-yc)*(j-yc))<5){
				// 	phi[i][j][k] = 0;
				// }
				...
				phi[i][j][k] *= 1.0;
				//noise
				phi[i][j][k] += 0.05*(1.0-2.0*drand48());		
	  ...
	  for (j = 0; j < Ly; j++) {
	    for (k = 0; k < Lz; k++) {
	      ...
	      */
	      ...
	      freeE += -Abulk*gammac/3.0*(Qsqxx*Qxxl + Qsqyy*Qyyl + Qsqzz*Qzzl + 3*Qxxl*Qsqxy + 3*Qxxl*Qsqxz + 3*Qyyl*Qsqxy + 3*Qyyl*Qsqyz + 3*Qzzl*Qsqxz + 3*Qzzl*Qsqyz + 6*Qxyl*Qyzl*Qxzl);
        freeE += L1/2.0*(dQxxdx*dQxxdx + dQxxdy*dQxxdy + dQxxdz*dQxxdz);
	      freeE += L1*(dQxydx*dQxydx + dQxydy*dQxydy + dQxydz*dQxydz);
	      freeE += L1*(dQxzdx*dQxzdx + dQxzdy*dQxzdy + dQxzdz*dQxzdz);
	      freeE += L1/2.0*(dQyydx*dQyydx + dQyydy*dQyydy + dQyydz*dQyydz);
	      freeE += L1*(dQyzdx*dQyzdx + dQyzdy*dQyzdy + dQyzdz*dQyzdz);
        freeE += L1/2.0*(dQzzdx*dQzzdx + dQzzdy*dQzzdy + dQzzdz*dQzzdz);
	      /* total free energy */
	      anchoring_energy += W*(Qxxl*dphidx*dphidx + Qyyl*dphidy*dphidy + Qzzl*dphidz*dphidz + 2*Qxyl*dphidx*dphidy + 2*Qxzl*dphidx*dphidz + 2*Qyzl*dphidz*dphidy); 
	      total_energy += freeE + energy + anchoring_energy;
...
/* work out antisymmetric part of the stress tensor, this will go to the bodyforce */
	...
	total_energy = total_energy/(Lx*Ly*Lz);
	anchoring_energy = anchoring_energy/(Lx*Ly*Lz);
...
void plotQ(int n)
{
	...
	if(movie) sprintf(filename1, "Qtensor_phi0_%0.3f_gamma0_%0.3f_k_%0.3f_W_%0.3f_%d.txt",phi0,gamma0,Kphi,W,n);
	else sprintf(filename1, "Qtensor_phi0_%0.3f_gamma0_%0.3f_k_%0.3f_W_%0.3f.txt",phi0,gamma0,Kphi,W);
		...
		for (j = 0; j < Ly; j++) {
			for (k = 0; k < Lz; k++) {
			  ...
			  if (d[2] > d[emax]) {
			    ...
			    enxt=2; // i think there is a bug here? but no biggy
			    }

			  /*assign the values to the director field*/
				n_director[i][j][k][0] = v[0][emax];
				n_director[i][j][k][1] = v[1][emax];
				n_director[i][j][k][2] = v[2][emax];
...
void computebinder(void)
{
  ...
  U = 1-phi4/(3*phi2*phi2);
}

// Helper to wrap nematic angle differences
double wrap_nematic_angle_difference(double dtheta) {
    if (dtheta > Pi / 2.0)
        dtheta -= Pi;
...
double computedefectdensity(void)
{
	...
	for(i=0 ; i<Lx; i++) {
		for(j=0 ; j<Ly; j++) {
			double nx = n_director[i][j][0][0];
			double ny = n_director[i][j][0][1];
			theta[i][j] = atan2(ny, nx); 
		}
	}
	...
	for(i=0 ; i<Lx; i++) {
		...
		for(j=0 ; j<Ly; j++) {
			int jp = (j+1)%Ly;
			double theta_ij   = theta[i][j];
			double thetaR     = theta[ip][j];
			double thetaT     = theta[i][jp];
			double thetaTR    = theta[ip][jp];

			// Counter-clockwise loop differences
			double dtheta1 = wrap_nematic_angle_difference(thetaR - theta_ij);
			double dtheta2 = wrap_nematic_angle_difference(thetaTR - thetaR);
			double dtheta3 = wrap_nematic_angle_difference(thetaT - thetaTR);
			double dtheta4 = wrap_nematic_angle_difference(theta_ij - thetaT);

			double charge = (dtheta1 + dtheta2 + dtheta3 + dtheta4)/(2.0*Pi);
			if(fabs(charge)<0.4) counter++;
		}
	}

	return counter/(Lx*Ly);
}
